#!/usr/bin/env bash
# ============================================================
#  Fatura Pro - Late Payment Toolkit + gold announcement bar
#  Adds:
#   1) src/pages/LatePaymentScripts.jsx  -> public page /late-payment-scripts
#   2) src/TopBar.jsx                    -> dismissible gold bar at the top
#   3) route + <TopBar /> in src/App.js
#   4) footer link on the landing page
#   5) /late-payment-scripts in public/sitemap.xml
#  Safe to run twice. Creates .backup copies of every edited file.
# ============================================================
set -e
...
cp src/pages/Landing.jsx src/pages/Landing.jsx.backup
cp public/sitemap.xml public/sitemap.xml.backup

echo "-> writing src/pages/LatePaymentScripts.jsx"
cat > src/pages/LatePaymentScripts.jsx <<'FATURA_PAGE_EOF'
...
echo "-> writing src/TopBar.jsx"
cat > src/TopBar.jsx <<'FATURA_TOPBAR_EOF'
...
echo "-> wiring src/App.js"
if grep -q "LatePaymentScripts" src/App.js; then
  echo "   (App.js already wired, skipping)"
else
  awk '{print} /^import ApiDocs from/ {print "import LatePaymentScripts from \"./pages/LatePaymentScripts\";"} /^import InstallPrompt from/ {print "import TopBar from \"./TopBar\";"}' src/App.js > src/App.js.tmp && mv src/App.js.tmp src/App.js
  awk '{print} /path="\/api-docs"/ {print "      <Route path=\"/late-payment-scripts\" element={<LatePaymentScripts />} />"}' src/App.js > src/App.js.tmp && mv src/App.js.tmp src/App.js
  awk '{print} /<AppRoutes \/>/ {print "      <TopBar />"}' src/App.js > src/App.js.tmp && mv src/App.js.tmp src/App.js
fi

echo "-> adding the footer link on the landing page"
if grep -q "late-payment-scripts" src/pages/Landing.jsx; then
  echo "   (footer link already there, skipping)"
else
  awk -v old='["/api-docs","API Documentation"]' -v new='["/api-docs","API Documentation"],["/late-payment-scripts","Late Payment Scripts"]' '{ if(!done){ i=index($0,old); if(i>0){ $0=substr($0,1,i-1) new substr($0,i+length(old)); done=1 } } print }' src/pages/Landing.jsx > src/pages/Landing.jsx.tmp && mv src/pages/Landing.jsx.tmp src/pages/Landing.jsx
fi

echo "-> updating public/sitemap.xml"
if grep -q "late-payment-scripts" public/sitemap.xml; then
  echo "   (sitemap already updated, skipping)"
else
  awk '/<\/urlset>/ && !d { print "  <url>"; print "    <loc>https://faturapro.app/late-payment-scripts</loc>"; print "    <changefreq>monthly</changefreq>"; print "    <priority>0.9</priority>"; print "  </url>"; d=1 } {print}' public/sitemap.xml > public/sitemap.xml.tmp && mv public/sitemap.xml.tmp public/sitemap.xml
fi

echo
echo "-> verification"
grep -q "import LatePaymentScripts" src/App.js        && echo "   OK  page imported"
grep -q "import TopBar" src/App.js                    && echo "   OK  top bar imported"
grep -q 'path="/late-payment-scripts"' src/App.js     && echo "   OK  route added"
grep -q "<TopBar />" src/App.js                       && echo "   OK  top bar mounted"
grep -q "late-payment-scripts" src/pages/Landing.jsx  && echo "   OK  footer link added"
grep -q "late-payment-scripts" public/sitemap.xml     && echo "   OK  sitemap updated"

echo
echo "DONE."
echo "Test locally:  npm start   then open  http://localhost:3000/late-payment-scripts"
echo "The gold bar shows on the home page, blog and other public pages (never inside the app)."
echo
echo "When it looks right:"
echo "  git add -A"
echo "  git commit -m \"Add Late Payment Toolkit page and site-wide announcement bar\""
echo "  git push"
