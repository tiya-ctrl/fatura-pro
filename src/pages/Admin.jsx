...
function ApplicationCard({ application, policy, onChanged, onNotice }) {
  ...
  const approve = async () => {
    ...
    try {
      const result = await approveAmbassador(application.id);
  ...
  return <article className={`admcard${pending ? "" : " admarchive"}`}><div className="admcardtop"><div><h3>{application.name}</h3><div className="admmuted">{application.email} · {application.primary_channel} · {application.audience_size} · {application.country}</div></div><span className="admbadge">{application.status}</span></div><p className="admwhy">{application.motivation}</p><a className="admprofile admmuted" href={application.profile_url} target="_blank" rel="noreferrer">Open public profile →</a>{pending && <><Policy policy={policy} /><div className="admactions"><button className="admbtn" disabled={!!busy} onClick={approve}>{busy === "approve" ? "Approving…" : "Approve ambassador"}</button><button className="admbtn danger" disabled={!!busy} onClick={decline}>{busy === "decline" ? "Declining…" : "Decline"}</button><span className="admmuted">Terms are automatic. The applicant must have a Fatūra Pro account with this email.</span></div></>}{application.status === "approved" && <div className="admactions"><button className="admbtn secondary" disabled={!!busy} onClick={resend}>{busy === "resend" ? "Sending…" : "Resend acceptance email"}</button><span className="admmuted">Includes the personal referral link and private tracking dashboard.</span></div>}{error && <div className="admerror">{error}</div>}</article>;
}
...
export default function Admin() {
  ...
  return <div className="adm"><style>{CSS}</style><header className="admwrap admnav"><strong>Fatūra Pro · Partner control</strong><a href="/app">Open app →</a></header><main className="admwrap admmain"><div className="admkicker">Private administration</div><h1 className="admtitle">Ambassadors</h1><p className="admsub">New applications arrive here. Approval creates the tracking account, activates automatic plan-based commission and emails the ambassador immediately.</p><Policy policy={policy} />{error && <div className="admerror">{error}</div>}{notice && <div className="admnotice">{notice}</div>}
