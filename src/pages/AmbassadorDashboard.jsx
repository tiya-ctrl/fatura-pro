  ...
  return (
    <div className="apage"><style>{CSS}</style>
      ...
      <main className="awrap amain">
        ...
        <section className="asection"><h2>Your automatic commission rules</h2><p>These are the terms approved for your partnership. The customer’s subscription plan selects the matching rate automatically.</p><div className="aterms"><div><strong>{data.account.commissionByPlan?.pro?.commissionPercent ?? 25}%</strong><span>Eligible Pro subscription revenue, excluding tax</span></div><div><strong>{data.account.commissionByPlan?.business?.commissionPercent ?? 35}%</strong><span>Eligible Business subscription revenue, excluding tax</span></div><div><strong>{data.account.commissionMonths} months</strong><span>From each customer’s first successful paid subscription</span></div></div><div className="anotice">After each customer’s first {data.account.commissionMonths} paid months, future payments create no commission. The stop is automatic. Fatūra Pro can also pause or end a partnership for the reasons in the program terms.</div></section>

        <section className="asection"><h2>Payouts</h2><p>Pending: <b>{euros(earnings.pending)}</b> · Ready: <b>{euros(earnings.available)}</b> · Processing: <b>{euros(earnings.processing)}</b>. Automatic transfer starts at {euros(data.account.payoutThresholdCents)} after the holding period.</p>{data.account.recoveryCents > 0 && <div className="anotice">A {euros(data.account.recoveryCents)} payment-reversal or dispute adjustment will be offset against future available commission before the next transfer.</div>}<div className="alinkrow">{data.account.payoutConnected ? <><button className="abtn secondary" disabled={busy} onClick={() => stripeAction("dashboard")}>Open Stripe payout details →</button><span className="astatus">● Bank payout connected</span></> : <button className="abtn" disabled={busy} onClick={() => stripeAction("connect")}>{busy ? "Opening Stripe…" : "Connect bank payout →"}</button>}</div>
