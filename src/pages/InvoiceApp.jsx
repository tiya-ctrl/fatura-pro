        ...
        {step === 1 && (
          <div>
            ...
            <div className="form-grid">
              ...
              <div className="form-group"><label>Country</label><select value={COUNTRY_OPTIONS.some(([c]) => c === (form.buyerCountry || countryCodeFrom(form.buyerAddress))) ? (form.buyerCountry || countryCodeFrom(form.buyerAddress)) : "OTHER"} onChange={e => set("buyerCountry", e.target.value === "OTHER" ? "" : e.target.value)}>{COUNTRY_OPTIONS.map(([c, n]) => <option key={c} value={c}>{n}</option>)}<option value="OTHER">Other…</option></select>{!COUNTRY_OPTIONS.some(([c]) => c === (form.buyerCountry || countryCodeFrom(form.buyerAddress))) && <input placeholder="Client country or code (e.g. JP)" value={form.buyerCountry || ""} onChange={e => set("buyerCountry", e.target.value)} style={{ marginTop:6 }} />}</div><div className="form-group"><label>Invoice Date *</label><input type="date" value={form.date} onChange={e => set("date", e.target.value)} /></div>
