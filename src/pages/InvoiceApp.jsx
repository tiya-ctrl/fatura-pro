...
const STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .mobile-fab { display: none; position: fixed; bottom: 80px; right: 20px; z-index: 25;
    ...
    box-shadow: 0 4px 20px rgba(201,168,76,0.45); display: none; align-items: center; justify-content: center; }
  ...
  /* Mobile invoice cards (replaces table on small screens) */
  ...
  /* ── INVOICE PREVIEW ── */
  .invoice-preview {
    background: #fff; color: #1a1a2e; border-radius: 12px; padding: 40px 48px;
  ...
  .tabs { display: flex; gap: 3px; background: var(--bg2); border: 1px solid var(--border);
    border-radius: 10px; padding: 4px; overflow-x: auto; -webkit-overflow-scrolling: touch; }
  ...
  @media print {
    ...
    .invoice-preview { box-shadow: none; margin: 0; border-radius: 0; }
  }
`;

const INIT_INVOICES = [
  { id: "INV-001", client: "Karim Design Studio", email: "karim@design.ma", amount: 4500, date: "2026-05-01", due: "2026-05-31", status: "paid", items: [] },
  { id: "INV-002", client: "TechFlow Solutions", email: "info@techflow.com", amount: 12800, date: "2026-05-05", due: "2026-06-04", status: "pending", items: [] },
  { id: "INV-003", client: "Sara Beauty Brand", email: "sara@beauty.com", amount: 2200, date: "2026-04-15", due: "2026-05-14", status: "overdue", items: [] },
  { id: "INV-004", client: "Nour Events", email: "nour@events.dz", amount: 7650, date: "2026-05-10", due: "2026-06-09", status: "pending", items: [] },
  { id: "INV-005", client: "Atlas Commerce", email: "contact@atlas.ma", amount: 18400, date: "2026-05-15", due: "2026-06-14", status: "draft", items: [] },
];

const INIT_CLIENTS = [
  { id: 1, name: "Karim Design Studio", email: "karim@design.ma", phone: "+212 6XX XXX XXX", country: "Morocco", invoices: 4, total: 18200 },
  { id: 2, name: "TechFlow Solutions", email: "info@techflow.com", phone: "+33 6XX XXX XXX", country: "France", invoices: 2, total: 25600 },
  { id: 3, name: "Sara Beauty Brand", email: "sara@beauty.com", phone: "+213 6XX XXX XXX", country: "Algeria", invoices: 3, total: 9800 },
  { id: 4, name: "Nour Events", email: "nour@events.dz", phone: "+213 7XX XXX XXX", country: "Algeria", invoices: 1, total: 7650 },
];
...
const fmtCurrency = (n, currencyCode = "EUR") => {
  ...
  const num = Number(n).toLocaleString(cur.locale, { minimumFractionDigits: noDecimals ? 0 : 2, maximumFractionDigits: noDecimals ? 0 : 2 });
  return isRTL ? `${num} ${cur.symbol}` : `${cur.symbol}${num}`;
};

const statusBadge = (s) => {
  const map = { paid: ["badge-paid","✓ Paid"], pending: ["badge-pending","● Pending"], overdue: ["badge-overdue","⚠ Overdue"], draft: ["badge-draft","✎ Draft"] };
  const [cls, label] = map[s] || ["badge-draft", s];
  return <span className={`badge ${cls}`}>{label}</span>;
};

const fmt = (n) => fmtCurrency(n, "EUR"); // legacy fallback
...
export default function InvoiceApp({ onGoHome }) {
  ...
  const [invoiceDraft, setInvoiceDraft] = useState(null); // auto-saved draft
useEffect(() => {
  if (invoiceDraft) {
    localStorage.setItem("invoice_draft_data", JSON.stringify(invoiceDraft));
  }
}, [invoiceDraft]);
  ...
  // Called when modal closes — saves draft if data exists
  ...
  const [remindersLog, setRemindersLog] = useState({}); // { invoiceId: [{date, tone, channel}] }
  ...
  const updateInvoice = (inv) => { setInvoices(prev => prev.map(i => i.id === inv.id ? inv : i)); setEditingInvoice(null); };
      ...
      <div className="app">
        ...
        <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
          <div className="logo">
            ...
            {onGoHome && <button onClick={onGoHome} style={{marginLeft:"auto",background:"none",border:"none",color:"var(--text2)",cursor:"pointer",fontSize:18}}>🏠</button>}
          </div>
              ...
              <div>
                <div style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px 14px", marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 2 }}>
                    {invoices.length}/5 invoices · {clients.length}/3 clients
                  </div>
                  <div style={{ marginTop: 8, background: "var(--bg4)", borderRadius: 4, height: 4, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${Math.min(100, (invoices.length/5)*100)}%`, background: invoices.length >= 5 ? "var(--red)" : "var(--gold)", borderRadius: 4, transition: "width 0.3s" }} />
                  </div>
                </div>
        ...
        {page === "invoices" && (
          <button className="mobile-fab" onClick={openNewInvoice}>+</button>
        )}
        ...
        {editingInvoice && <NewInvoiceModal clients={clients} onSave={updateInvoice} onClose={(draftData) => { if (draftData) setInvoiceDraft(draftData); setEditingInvoice(null); }} invoiceCount={invoices.length} currency={currency} f={f} editData={editingInvoice} />}
...
function Dashboard({ invoices, totalRevenue, totalPending, totalOverdue, setPage, setPreviewInvoice, onEdit, onRemind, f }) {
  ...
  return (
    <>
      ...
      {overdue.length > 0 && (
        <div style={{ background: "rgba(224,85,85,0.1)", border: "1px solid rgba(224,85,85,0.35)", borderRadius: 10,
          padding: "12px 20px", marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "var(--text2)", marginTop: 2 }}>
                {overdue.map(i => i.client).join(", ")} — Total: <strong style={{ color: "var(--red)" }}>{f(overdue.reduce((a,b)=>a+b.amount,0))}</strong>
              </div>
            </div>
          </div>
          <button className="btn btn-sm" style={{ background: "rgba(224,85,85,0.2)", color: "var(--red)", border: "1px solid rgba(224,85,85,0.4)", whiteSpace: "nowrap" }}
            onClick={() => setPage("invoices")}>View Overdue →</button>
        </div>
      )}
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="stat-value">{f(totalRevenue)}</div>
          <div className="stat-change">↑ +12.4% this month</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">⏳</div>
          <div className="stat-label">Pending</div>
          <div className="stat-value">{f(totalPending)}</div>
          <div className="stat-change">{invoices.filter(i => i.status === "pending").length} invoices</div>
        </div>
        <div className="stat-card">
          <div className="stat-icon">⚠️</div>
          <div className="stat-label">Overdue</div>
          <div className="stat-value" style={{ color: "var(--red)" }}>{f(totalOverdue)}</div>
              ...
              <tr key={inv.id}>
                ...
                <td>
                  <div className="action-btns">
                    ...
                    <button className="btn btn-ghost btn-sm" style={{ color: "var(--gold)" }} onClick={() => onEdit(inv)}>✏️ Edit</button>
...
function Invoices({ invoices, filterStatus, setFilterStatus, search, setSearch, onPreview, onDelete, onNew, onEdit, onRemind, remindersLog, f, isPro, onUpgrade, hasDraft, onOpenDraft, onDiscardDraft }) {
  const statuses = ["all", "paid", "pending", "overdue", "draft"];
  return (
    <>
      {/* Draft recovery banner */}
      {hasDraft && (
        <div style={{ background: "rgba(201,168,76,0.1)", border: "1px solid var(--gold)", borderRadius: 10,
          padding: "12px 18px", marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 20 }}>📝</span>
            <div>
              <div style={{ fontWeight: 700, color: "var(--gold)", fontSize: 13 }}>You have an unsaved draft</div>
      ...
      <div className="card">
        ...
        <table>
          <thead><tr>
            <th>Invoice #</th><th>Client</th><th>Date</th><th>Due Date</th><th>Amount</th><th>Status</th><th>Actions</th>
          </tr></thead>
          <tbody>
            {invoices.length === 0 ? (
              <tr><td colSpan={7}><div className="empty"><div className="empty-icon">📋</div><div className="empty-text">No invoices found</div></div></td></tr>
            ) : invoices.map((inv) => (
              <React.Fragment key={inv.id}>
                <tr>
                  ...
                  <td>
                    ...
                    {remindersLog[inv.id]?.length > 0 && (
                      <div style={{ fontSize: 10, color: "var(--orange)", marginTop: 2 }}>
                        🔔 {remindersLog[inv.id].length} reminder{remindersLog[inv.id].length > 1 ? "s" : ""} sent
                      </div>
                    )}
                  </td>
                  ...
                  <td>
                    <div className="action-btns">
                      <button className="btn btn-ghost btn-sm" onClick={() => onPreview(inv)}>👁</button>
                      <button className="btn btn-ghost btn-sm" style={{ color: "var(--gold)" }} onClick={() => onEdit(inv)}>✏️</button>
                      {(inv.status === "overdue" || inv.status === "pending") && (
        ...
        <div className="inv-cards" style={{ padding: invoices.length ? "12px" : 0 }}>
          {invoices.length === 0
            ? <div className="empty"><div className="empty-icon">📋</div><div className="empty-text">No invoices found</div></div>
            : invoices.map(inv => (
              <div className="inv-card" key={inv.id}>
                ...
                <div className="inv-card-row">
                  ...
                  <div className="inv-card-due" style={{ color: inv.status === "overdue" ? "var(--red)" : "var(--text2)", fontWeight: inv.status === "overdue" ? 700 : 400 }}>
                    Due: {inv.due}
                  </div>
                </div>
                {remindersLog[inv.id]?.length > 0 && (
      ...
      {clients.map(c => (
        <div className="client-card" key={c.id}>
          ...
          <div className="client-stats" style={{ marginTop: 14 }}>
            ...
            <div className="client-stat">
              <span style={{ color: "var(--gold)" }}>{f(c.total)}</span>
              Total Billed
            </div>
          </div>
        </div>
    ...
    <div style={{ maxWidth: 600 }}>
      <div className="card" style={{ padding: 28, marginBottom: 20 }}>
        ...
        <div className="form-grid">
          ...
          <div className="form-group"><label>Phone</label><input defaultValue="+33 6 XX XX XX XX" /></div>
          ...
          <div className="form-group full"><label>Address</label><input defaultValue="123 Rue de la Paix, Paris" /></div>
        </div>
        ...
        <div className="form-grid">
          <div className="form-group full">
            ...
            <div style={{ fontSize: 12, color: "var(--gold)", marginTop: 6, padding: "8px 12px", background: "var(--gold-dim)", borderRadius: 6, border: "1px solid var(--border)" }}>
              ✦ Preview: {fmtCurrency(1234.5, currency)} · Selected: <strong>{cur.label}</strong>
            </div>
          </div>
...
function NewInvoiceModal({ clients, onSave, onClose, invoiceCount, currency: globalCurrency, f: globalF, editData, draftData, onDiscardDraft }) {
  const isEdit = !!editData;
  const sourceData = editData || draftData; // use draft if no edit
  const [showDraftBanner, setShowDraftBanner] = useState(!!draftData && !editData);
  const [step, setStep] = useState(0);
  const [invoiceCurrency, setInvoiceCurrency] = useState(sourceData?.currency || globalCurrency || "EUR");
  const fLocal = (n) => fmtCurrency(n, invoiceCurrency);
  const curInfo = getCurrency(invoiceCurrency);

  const [sellerLogoSize, setSellerLogoSize] = useState(sourceData?.sellerLogoSize || 80);
  const [buyerLogoSize, setBuyerLogoSize] = useState(sourceData?.buyerLogoSize || 60);

  const emptyForm = {
    sellerName: "", sellerEmail: "", sellerPhone: "+31", sellerAddress: "", sellerLogo: null,
    client: clients[0]?.name || "", email: clients[0]?.email || "",
  ...
  const [items, setItems] = useState(
    sourceData?.items?.length > 0 ? sourceData.items : [{ desc: "", qty: 1, price: 0 }]
  );
  ...
  const handleClientChange = (name) => {
    ...
    setForm(f => ({ ...f, client: name, email: c?.email || "", buyerPhone: c?.phone || "", buyerAddress: "" }));
  };
  ...
  const LogoUploader = ({ logoKey, sizeVal, onSizeChange, icon, inputId, label }) => (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text2)", marginBottom: 10 }}>{label}</div>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
        <div style={{ width: 90, height: 90, borderRadius: 10, background: "var(--bg3)", border: "2px dashed var(--border)",
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            ...
            {form[logoKey] && (
              <button className="btn btn-danger btn-sm" onClick={() => set(logoKey, null)}>Remove</button>
            )}
          </div>
          {form[logoKey] ? (
            <div>
              <div style={{ fontSize: 11, color: "var(--text2)", marginBottom: 6 }}>
                Logo Size: <strong style={{ color: "var(--gold)" }}>{sizeVal}%</strong>
              </div>
              <input type="range" min={20} max={100} value={sizeVal}
      ...
      <div className="modal" style={{ maxWidth: 700 }}>
          ...
          <div style={{ background: "rgba(201,168,76,0.12)", border: "1px solid var(--gold)", borderRadius: 10,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text2)" }}>You have an unsaved invoice. Continue where you left off?</div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
          <div className="modal-title" style={{ marginBottom: 0 }}>
            {isEdit ? "✏️ Edit Invoice" : "✦ New Invoice"}
            {isEdit && <span style={{ fontSize: 13, color: "var(--gold)", marginLeft: 10, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>{editData.id}</span>}
          </div>
          {/* Currency selector — prominent gold pill */}
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
            ...
            <div style={{ position: "relative" }}>
              <select value={invoiceCurrency} onChange={e => setInvoiceCurrency(e.target.value)}
                style={{ background: "var(--gold-dim)", border: "1.5px solid var(--gold)", color: "var(--gold)",
                  fontWeight: 700, fontSize: 13, borderRadius: 8, padding: "7px 32px 7px 12px",
          ...
          {steps.map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", flex: i < steps.length - 1 ? 1 : "none" }}>
              <div onClick={() => i < step && setStep(i)} style={{
                ...
                fontSize: 13, fontWeight: 700, flexShrink: 0, cursor: i < step ? "pointer" : "default", transition: "all 0.2s",
        ...
        {step === 0 && (
          <div>
            <LogoUploader logoKey="sellerLogo" sizeVal={sellerLogoSize} onSizeChange={setSellerLogoSize}
              icon="🏢" inputId="sellerLogoInput" label="Company / Seller Logo" />
            <div className="form-grid">
              <div className="form-group full">
                <label>Seller / Company Name</label>
                <input value={form.sellerName} onChange={e => set("sellerName", e.target.value)} placeholder="e.g. Vyynd Agency BV" />
              </div>
              <div className="form-group">
                <label>Email</label>
                <input value={form.sellerEmail} onChange={e => set("sellerEmail", e.target.value)} placeholder="contact@yourcompany.com" />
              </div>
              <div className="form-group">
                <label>Phone</label>
                <input value={form.sellerPhone} onChange={e => set("sellerPhone", e.target.value)} placeholder="+31 6 XX XX XX XX" />
              </div>
              <div className="form-group full">
                <label>Address</label>
                <textarea rows={2} value={form.sellerAddress} onChange={e => set("sellerAddress", e.target.value)}
                  placeholder="e.g. Keizersgracht 123, Amsterdam, Netherlands" style={{ resize: "none" }} />
              </div>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            <LogoUploader logoKey="buyerLogo" sizeVal={buyerLogoSize} onSizeChange={setBuyerLogoSize}
              icon="👤" inputId="buyerLogoInput" label="Client / Buyer Logo (optional)" />
            <div className="form-grid">
              <div className="form-group full">
              ...
              <div className="form-group full">
                ...
                <input value={form.client} onChange={e => set("client", e.target.value)} placeholder="e.g. TechFlow Solutions BV" />
              </div>
              <div className="form-group">
                <label>Email</label>
                <input value={form.email} onChange={e => set("email", e.target.value)} placeholder="client@company.com" />
              </div>
              <div className="form-group">
                <label>Phone</label>
                <input value={form.buyerPhone} onChange={e => set("buyerPhone", e.target.value)} placeholder="+971 50 XXX XXXX" />
              </div>
              <div className="form-group full">
                <label>Address</label>
                <textarea rows={2} value={form.buyerAddress} onChange={e => set("buyerAddress", e.target.value)}
                  placeholder="e.g. Sheikh Zayed Rd, Dubai, UAE" style={{ resize: "none" }} />
              </div>
              <div className="form-group">
        ...
        {step === 2 && (
          <div>
            {/* Column headers */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 100px 28px", gap: 8,
              padding: "6px 12px", background: "var(--bg3)", borderRadius: 8, border: "1px solid var(--border)", marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12 }}>
              {items.map((it, idx) => (
                <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 100px 28px", gap: 8,
                  padding: "8px 12px", background: "var(--bg4)", borderRadius: 8, border: "1px solid var(--border)", alignItems: "center" }}>
                  <input value={it.desc} onChange={e => updateItem(idx, "desc", e.target.value)}
                    placeholder="e.g. Social Media Package"
                    style={{ width: "100%", padding: "8px 10px", fontSize: 13 }} />
                  <input type="number" value={it.qty === 0 ? "" : it.qty} min={0}
                    onChange={e => updateItem(idx, "qty", e.target.value === "" ? 0 : +e.target.value)}
                    placeholder="1"
                    style={{ width: "100%", padding: "8px 10px", fontSize: 14, fontWeight: 600, textAlign: "center" }} />
                  <input type="number" value={it.price === 0 ? "" : it.price} min={0}
                    onChange={e => updateItem(idx, "price", e.target.value === "" ? 0 : +e.target.value)}
                    placeholder="0.00"
                    style={{ width: "100%", padding: "8px 10px", fontSize: 14, fontWeight: 600, textAlign: "right" }} />
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--gold)", textAlign: "right" }}>
                    {fLocal(it.qty * it.price)}
                  </div>
                  <button onClick={() => removeItem(idx)}
                    style={{ background: "none", border: "none", color: "var(--red)", cursor: "pointer", fontSize: 18, lineHeight: 1, padding: 0 }}>×</button>
                </div>
              ))}
            </div>
            ...
            <div className="form-grid" style={{ gridTemplateColumns: "1fr 1fr", marginBottom: 0 }}>
              <div className="form-group">
                ...
                <input type="number" value={form.discount === 0 ? "" : form.discount} min={0} max={100}
                  onChange={e => set("discount", e.target.value === "" ? 0 : +e.target.value)} placeholder="0" />
              </div>
              <div className="form-group">
                <label>Tax / VAT (%)</label>
                <input type="number" value={form.tax === 0 ? "" : form.tax} min={0}
                  onChange={e => set("tax", e.target.value === "" ? 0 : +e.target.value)} placeholder="20" />
              </div>
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 18px" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text2)", letterSpacing: 0.5, marginBottom: 10, textTransform: "uppercase" }}>
                {isEdit ? "✏️ Editing Invoice" : "📋 Summary"}
              </div>
              {[
                ["Seller", form.sellerName || "—"],
                ["Client", form.client || "—"],
                ["Due Date", form.due || "—"],
                ["Items", `${items.filter(i => i.desc).length} line item(s)`],
              ].map(([k, v]) => (
  ...
  alert("تم حفظ المسودة بنجاح!");
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            {!isLast && (
              <span style={{ fontSize: 12, color: "var(--text2)" }}>Step {step + 1} / {steps.length}</span>
            )}
            {isLast
              ? <button className="btn btn-primary" onClick={handleSave}>
                  {isEdit ? "✏️ Update Invoice" : "✦ Save Invoice"}
                </button>
        ...
        <div className="form-grid" style={{ gridTemplateColumns: "1fr" }}>
          ...
          <div className="form-group">
            ...
            <select value={form.country} onChange={e => setForm(f => ({ ...f, country: e.target.value }))}>
              {["Morocco", "Algeria", "Tunisia", "France", "Belgium", "UAE", "Saudi Arabia", "Other"].map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
        </div>
...
function InvoicePreview({ invoice, onClose, currency }) {
  ...
  const taxAmt = invoice.taxAmt ?? invoice.amount * 0.2;
  const discountAmt = invoice.discountAmt ?? 0;
  const total = invoice.total ?? invoice.amount * 1.2;
  const tax = invoice.tax ?? 20;
  const discount = invoice.discount ?? 0;

  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ width: "100%", maxWidth: 760, maxHeight: "95vh", overflow: "auto", borderRadius: 16 }}>
        ...
        <div className="invoice-preview">
          ...
          {/* ── HEADER: Logos + Invoice Number ── */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 36 }}>
            {/* Seller logo or name */}
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              {invoice.sellerLogo
                ? <img src={invoice.sellerLogo} style={{ height: 56, maxWidth: 140, objectFit: "contain" }} alt="seller logo" />
                : <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, color: "#c9a84c", fontWeight: 700, letterSpacing: 0.5 }}>
                    {invoice.sellerName || "Fatūra"}
                  </div>
              }
            </div>
            {/* Invoice meta */}
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 12, color: "#777", marginTop: 6 }}>
                <div><span style={{ fontWeight: 600 }}>Date:</span> {invoice.date}</div>
                <div><span style={{ fontWeight: 600 }}>Due:</span> {invoice.due}</div>
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 36,
            background: "#faf8f3", borderRadius: 10, padding: "20px 24px" }}>
            {/* FROM */}
            <div>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#c9a84c", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10,
                display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ background: "#c9a84c", color: "#fff", borderRadius: 4, padding: "2px 7px", fontSize: 9 }}>FROM</span>
                Seller
              </div>
              {invoice.sellerLogo && <img src={invoice.sellerLogo} style={{ height: 36, objectFit: "contain", marginBottom: 8, display: "block" }} alt="" />}
            ...
            <div style={{ borderLeft: "1px solid #e8dfc8", paddingLeft: 24 }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#555", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10,
                display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ background: "#555", color: "#fff", borderRadius: 4, padding: "2px 7px", fontSize: 9 }}>TO</span>
                Client
              </div>
              {invoice.buyerLogo && <img src={invoice.buyerLogo} style={{ height: 36, objectFit: "contain", marginBottom: 8, display: "block" }} alt="" />}
          ...
          <table className="preview-table">
            ...
            <tbody>
              {(invoice.items?.length > 0 ? invoice.items : [{ desc: "Professional Services", qty: 1, price: invoice.amount }]).map((it, i) => (
                <tr key={i}>
                  <td>{it.desc || "Service"}</td>
                  <td>{it.qty}</td>
                  <td>{f(it.price)}</td>
          ...
          {/* ── NOTES ── */}
          {invoice.notes && (
            <div style={{ marginBottom: 16, padding: "14px 18px", background: "#f5f3ef", borderRadius: 8, borderLeft: "3px solid #c9a84c" }}>
          ...
          {/* ── BANK INFO ── */}
          {invoice.bankInfo && (
            <div style={{ padding: "14px 18px", background: "#f0f5ff", borderRadius: 8, borderLeft: "3px solid #4a7fc1" }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#4a7fc1", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>🏦 Payment Information</div>
...
function ReminderModal({ invoice, onClose, onLog, f }) {
  const today = new Date().toISOString().split("T")[0];
  const daysOverdue = invoice.due
    ? Math.floor((new Date(today) - new Date(invoice.due)) / 86400000)
    : 0;
  ...
  const templates = {
    polite: {
      subject: `Friendly Reminder – Invoice ${invoice.id}`,
      body: `Dear ${invoice.client},\n\nI hope this message finds you well.\n\nThis is a friendly reminder that Invoice ${invoice.id} for ${f(invoice.amount)} was due on ${invoice.due}.\n\nIf you have already processed this payment, please disregard this message. Otherwise, we kindly ask that you arrange the payment at your earliest convenience.\n\nPlease don't hesitate to reach out if you have any questions.\n\nWarm regards,\n${invoice.sellerName || "Your Company"}`,
    },
    firm: {
      subject: `Payment Overdue – Invoice ${invoice.id} (${daysOverdue} days)`,
      body: `Dear ${invoice.client},\n\nWe are writing to inform you that Invoice ${invoice.id} for ${f(invoice.amount)} is now ${daysOverdue} day${daysOverdue !== 1 ? "s" : ""} overdue.\n\nThe payment was due on ${invoice.due} and has not yet been received. We kindly request that you process this payment within the next 5 business days to avoid any further follow-up.\n\nIf there is an issue with the invoice, please contact us immediately so we can resolve it.\n\nBest regards,\n${invoice.sellerName || "Your Company"}`,
    },
    final: {
      subject: `FINAL NOTICE – Invoice ${invoice.id} Overdue ${daysOverdue} Days`,
      body: `Dear ${invoice.client},\n\nDespite our previous reminders, Invoice ${invoice.id} for ${f(invoice.amount)} remains unpaid as of today.\n\nThis invoice was due on ${invoice.due} and is now ${daysOverdue} days overdue. This is our final notice before we are required to take further action.\n\nPlease arrange full payment immediately or contact us within 48 hours to discuss a payment arrangement.\n\nWe value our business relationship and hope to resolve this matter promptly.\n\nRegards,\n${invoice.sellerName || "Your Company"}`,
    },
  };

  const whatsappTemplates = {
    polite: `Hi ${invoice.client} 👋 Just a friendly reminder about Invoice ${invoice.id} for ${f(invoice.amount)} which was due on ${invoice.due}. Please let us know if you have any questions. Thank you! 🙏`,
    firm: `Hi ${invoice.client}, this is a follow-up regarding Invoice ${invoice.id} for ${f(invoice.amount)}, now ${daysOverdue} days overdue (due: ${invoice.due}). Kindly process payment within 5 business days. Thank you.`,
    final: `Dear ${invoice.client}, Invoice ${invoice.id} for ${f(invoice.amount)} is ${daysOverdue} days overdue. This is our final notice. Please arrange immediate payment or contact us within 48 hours. — ${invoice.sellerName || "Your Company"}`,
  };
  ...
  const currentText = channel === "email" ? templates[tone].body : whatsappTemplates[tone];
  const [editedText, setEditedText] = useState(currentText);

  const handleToneChange = (t) => {
    setTone(t);
    setEditedText(channel === "email" ? templates[t].body : whatsappTemplates[t]);
  };
  const handleChannelChange = (c) => {
    setChannel(c);
    setEditedText(c === "email" ? templates[tone].body : whatsappTemplates[tone]);
  };

  const handleSend = () => {
    if (channel === "email" && invoice.email) {
      const subject = encodeURIComponent(templates[tone].subject);
      const body = encodeURIComponent(editedText);
      window.open(`mailto:${invoice.email}?subject=${subject}&body=${body}`, "_blank");
    } else if (channel === "whatsapp") {
      const phone = (invoice.buyerPhone || "").replace(/\D/g, "");
      const msg = encodeURIComponent(editedText);
  ...
  const handleCopy = () => {
    ...
    onLog(invoice.id, { date: today, tone, channel: "copy" });
    setSent(true);
  ...
  <div className="modal" style={{ maxWidth: 600 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text2)" }}>
              <span style={{ color: "var(--gold)", fontWeight: 700 }}>{invoice.id}</span> · {invoice.client} ·{" "}
              <span style={{ color: "var(--red)", fontWeight: 600 }}>{f(invoice.amount)}</span>
              {daysOverdue > 0 && <span style={{ marginLeft: 8, background: "rgba(224,85,85,0.15)", color: "var(--red)", borderRadius: 20, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>
        ...
        <div style={{ margin: "18px 0 14px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[["email", "✉️ Email"], ["whatsapp", "💬 WhatsApp"], ["copy", "📋 Copy Text"]].map(([id, label]) => (
              <button key={id} onClick={() => id === "copy" ? handleCopy() : handleChannelChange(id)}
                className="btn btn-sm"
                style={{ background: channel === id ? "var(--gold)" : "var(--bg3)", color: channel === id ? "#000" : "var(--text2)",
                  border: "1px solid var(--border)", fontWeight: channel === id ? 700 : 500 }}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {tones.map(t => (
              <div key={t.id} onClick={() => handleToneChange(t.id)}
                style={{ flex: 1, padding: "10px 12px", borderRadius: 10, cursor: "pointer", transition: "all 0.15s",
        ...
        <div className="form-group" style={{ marginBottom: 16 }}>
          <label>{channel === "email" && <span style={{ color: "var(--text2)", fontSize: 11 }}>Subject: <strong style={{ color: "var(--text)" }}>{templates[tone].subject}</strong></span>}</label>
          <textarea rows={channel === "email" ? 9 : 5}
            value={editedText}
            onChange={e => setEditedText(e.target.value)}
            style={{ resize: "vertical", fontSize: 12, lineHeight: 1.7, marginTop: channel === "email" ? 8 : 0 }} />
          ...
          {sent
            ...
            : <div style={{ display: "flex", gap: 8 }}>
                ...
                <button className="btn btn-primary" onClick={handleSend}
                  style={{ background: tone === "final" ? "var(--red)" : "var(--gold)", color: "#000" }}>
                  {channel === "email" ? "✉️ Open in Mail" : channel === "whatsapp" ? "💬 Open WhatsApp" : "📋 Copy & Log"}
                </button>
              </div>
          }
        </div>
...
function UpgradeModal({ feature, onClose, onActivate }) {
  ...
  const PLANS_INFO = {
    pro: {
      name: "Pro", price: "€9", period: "/month", color: "var(--gold)",
      features: ["Unlimited invoices", "Unlimited clients", "Payment reminders (Email + WhatsApp)", "PDF export", "Custom logo & branding"],
      stripe_link: "https://buy.stripe.com/test_YOUR_PRO_LINK", // ← replace with your Stripe link
    },
    business: {
      name: "Business", price: "€19", period: "/month", color: "#a78bfa",
      badge: "Coming Soon",
  ...
  const featureLabels = {
    ...
    pdf_export: { icon: "🖨️", label: "PDF Export", desc: "Download and share professional PDF invoices" },
  };

  const feat = featureLabels[feature] || { icon: "✦", label: "Pro Feature", desc: "Unlock all Pro features" };

  const handleStripe = () => {
    const plan = PLANS_INFO[selectedPlan];
    if (!plan.stripe_link) return;
    setLoading(true);
      ...
      <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="modal" style={{ maxWidth: 440, textAlign: "center" }}>
          ...
          <div style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 12, padding: "16px 20px", marginBottom: 24, textAlign: "left" }}>
            {PLANS_INFO.pro.features.map((f, i) => (
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 520 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div className="modal-title" style={{ textAlign: "center", marginBottom: 6 }}>
            Unlock <span style={{ color: "var(--gold)" }}>{feat.label}</span>
          </div>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
          {Object.entries(PLANS_INFO).map(([key, p]) => (
            <div key={key} onClick={() => !p.badge && setSelectedPlan(key)}
              style={{
                flex: 1, border: `1.5px solid ${selectedPlan === key ? p.color : "var(--border2)"}`,
                borderRadius: 12, padding: "16px 14px", cursor: p.badge ? "not-allowed" : "pointer",
                background: selectedPlan === key ? `${p.color}15` : "var(--bg3)",
              ...
              {p.badge && (
                <div style={{ position: "absolute", top: 8, right: 8, fontSize: 9, fontWeight: 700,
                  background: "var(--bg4)", color: "var(--text2)", borderRadius: 20, padding: "2px 7px", letterSpacing: 0.5 }}>
                  {p.badge}
                </div>
              )}
              ...
              {p.features.length > 3 && (
                <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 4 }}>+{p.features.length - 3} more features</div>
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 12, padding: "16px 18px", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
              {["💳", "🍎", "G"].map((ic, i) => (
                <div key={i} style={{ width: 28, height: 18, background: "var(--bg4)", border: "1px solid var(--border2)", borderRadius: 4,
                  display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>{ic}</div>
              ))}
            </div>
          </div>
        ...
        <button
          ...
          style={{ width: "100%", justifyContent: "center", fontSize: 15, padding: "14px", marginBottom: 10 }}
          onClick={handleStripe}
          disabled={loading || !!PLANS_INFO[selectedPlan].badge}>
          {loading
